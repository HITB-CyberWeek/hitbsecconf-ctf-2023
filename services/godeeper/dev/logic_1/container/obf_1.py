...
class Obfuscator:
    ...
    def GetExAddr(self,addr):
        b0 = addr // 255
        b1 = addr % 255
        if b0 >=3:
            b0 = addr // 256
            tmp = 255 - (addr % 256)
            if 255-b0 <= addr - 256*(addr//256) <255 :
                b1 = addr - 256*(addr//256) - (255-b0)
            else:
                b1 = addr - 256 * (addr // 256) + b0
        return (b1, b0)
    def ParseFile2(self):
    ...
    def ProduceFile2(self,fname):
        ...
        for fn in self.functions:
            ...
            b0,b1 = self.GetExAddr(start_offset)
            if b1 != 0:
    ...
    def SplitBlock(self,fn,block_idx):
        ...
        global MIN_LU_LEN
        if len(cur_block.lines) <= MIN_LU_LEN:
            return False
    ...
    def SwapBlocks(self):
        for fn in self.functions:
            ...
            for block in fn.blocks:
                ...
                if position % 255 == 0:
                    position +=4
                    block.lines = [FuncLine("NOP"),FuncLine("NOP")] + block.lines
